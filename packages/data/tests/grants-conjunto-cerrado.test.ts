...
const GRANTS_POR_COLUMNA: readonly {
  ...
  { tabla: 'reconocimiento_contrapartida', rol: 'app_request', privilegio: 'INSERT', columnas: ['cliente_id', 'padron_completo_hasta', 'padron_manifestacion_id', 'patron_contraparte_estado', 'patron_contraparte_origen', 'reconocimiento_clase', 'reconocimiento_id', 'resolucion_estado', 'resuelto_a_fecha'] },
  { tabla: 'reconocimiento_contrapartida', rol: 'app_request', privilegio: 'SELECT', columnas: ['admite_matches', 'admite_matches_patron', 'cliente_id', 'created_at', 'id', 'padron_completo_hasta', 'padron_manifestacion_id', 'patron_contraparte_estado', 'patron_contraparte_origen', 'reconocimiento_clase', 'reconocimiento_id', 'regimen_matches', 'regimen_matches_patron', 'resolucion_estado', 'resuelto_a_fecha'] },
...
describe('R41 — prueba de mutación: 13 mutaciones, elegidas para refutar', () => {
  ...
  it('🔴 M4: un grant a `app_job` sobre una tabla de 0021 lo pone rojo', async () => {
    await conGrantsMutados(['grant select on reconocimiento_contrapartida to app_job'], async (c) => {
      ...
      // 15, no 11: `0038` agregó 3 columnas a `reconocimiento_contrapartida`
      // (patron_contraparte_estado, admite_matches_patron, regimen_matches_patron) y `0039` agregó
      // 1 más (patron_contraparte_origen) — un grant de TABLA a `app_job` se expande a una fila de
      // `column_privileges` por columna, así que el conteo esperado sube 1:1 con el ancho real de
      // la tabla.
      expect(d.filter((x) => x.startsWith('POR_COLUMNA SOBRA:'))).toHaveLength(15);
    });
  });
