...
| **B.10** | ✅ **CERRADA por decisión explícita de JP (no confirmación de Laura), HANDOFF (156).** El plan de cuentas real de ROKA tiene 4 códigos ligados a persona (`1.2.4.300`, `1.2.4.400` en Activo; `2.1.9.100`, `2.1.9.200` en Pasivo — 1 código por persona, no pareado Activo+Pasivo como Bracci). De los 4 `padron_socio_id` reales cargados: **2 confirmados por evidencia documental real** (`1.2.4.300`=Gabriela, único alta de ROKA de la tanda de entrada 73; `1.2.4.400`="Cuenta Particular Socio 4"=el familiar NO-socio, resuelto en `privado/02-Consultas-Laura-2026-08-21.md` §1.5 pregunta 8, verificado por HMAC). **Los otros 2 (`2.1.9.100`=Martin Maria Laura, `2.1.9.200`=Martin Natalia) identificados por HMAC del CUIT documentado en la misma ronda de consultas contra `padron_socio.documento_hmac`** (sesión 2026-08-31/2026-09-01) — la identidad de CADA `padron_socio_id` es cierta; lo que nunca tuvo documento fue la posición "Socio 1"/"Socio 2" en el plan. JP decidió explícitamente fijar `2.1.9.100`=María Laura, `2.1.9.200`=Natalia como definitivo, sin esperar la confirmación de Laura. `respaldo` de las 2 filas actualizado en la base para documentar esta decisión (reemplaza la referencia al JSON de mapeo de scratchpad, ya inexistente) | Cerrada — sin acción pendiente. Si en el futuro apareciera evidencia documental de que el orden real era el inverso, el fix es un `UPDATE` puntual de `cuenta_atributo.padron_socio_id` para esas 2 filas, con su propio registro de auditoría — no una re-carga completa |
...
- ✅ **CERRADO — Módulo 1 (extracción) y Módulo 2 (clasificación), los dos.** Historia completa: el fix
  del vocabulario de Macro (`'ING TRANSF:'`, `packages/ingesta/src/adaptadores/macro.ts`,
  ...
  `concepto_banco` calculado con el vocabulario viejo, 671/699/714 movimientos por mes sin
  `conceptoBanco`, invisibles para Capa C. **Reproceso de extracción** (HANDOFF 166-167): se amplió
  ...
  671/699/714 filas actualizadas exacto, verificado por consulta directa.
  Al re-correr Capa C apareció un SEGUNDO hueco, distinto: las 671+699+714=2084 filas tenían concepto
  IDENTIFICADO pero caían en `concepto_no_catalogado` — el léxico de CLASIFICACIÓN de Módulo 2
  (`packages/contabilidad`) nunca tuvo una regla para `"ING TRANSF:"`. **Cerrado en (169)**: medí con
  `resolverContraparte()` sobre los 2084 movimientos reales (100% capturado en Módulo 1, 0% match
  contra el padrón de socios, homogéneo), convoqué a `contador-dominio` (Agent real) que confirmó
  reusar `macro.transferencia_recibida_de_terceros` (mismo hecho económico que `TPUSH`/`TRANSF`,
  `queDecide: 'distinguir_tercero_de_socio'`, D-31 sin excepción pese al 0% de socios en la muestra —
  protege el mes que sí traiga uno). Implementado con mutación verificada, revisado por
...
- ✅ **CERRADO — `asiento_propuesto_renglon.padron_contraparte_id` reclasificado N1→N2 (docs/diseno/
  33-plan-deuda-pre-tanda-4.md §A.3).** Estaba clasificado N1 (`UUID_INTERNO`) por una analogía que no
  era la correcta para su rol. Hallazgo original de `seguridad-datos-financieros`, convocatoria de la
  migración `0038` (escritor real de `reconocimiento_contrapartida` sobre `padron_contraparte`,
  `docs/diseno/29-padron-contraparte.md`). `0037` clasificó esa columna por analogía con
  `padron_manifestacion_id` ("el uuid no revela contenido") — pero `padron_manifestacion_id` es un
  puntero de VERSIÓN DE PROCESO que no identifica a nadie, y `padron_contraparte_id` cita la fila de UN
  proveedor/cliente puntual conocido por nombre. El precedente correcto, confirmado por `dba-data` y
  `seguridad-datos-financieros` en paralelo para la columna hermana de `0038`
  (`reconocimiento_contrapartida_patron_match.padron_contraparte_id`, N2): el mismo tier que
  `reconocimiento_contrapartida_match.socio_id`/`cuenta_atributo.padron_socio_id` — el riesgo es la
  correlación entre filas por un uuid recurrente (perfil comercial de ese tercero con este cliente), no
  el contenido del uuid. No bloqueaba `0038` (tablas distintas, columnas distintas).
  **Cierre aplicado**: edición pura de `packages/shared/src/seguridad/clasificacion-campos.ts` (nivel
  N1→N2 de esa entrada, más las dos notas hermanas que citaban el N1 viejo como precedente —
  `reconocimiento_contrapartida_patron_match.padron_contraparte_id` y
  `asiento_propuesto.corrige_asiento_id`), nunca de la migración `0037` ya aplicada. Efecto práctico
  inmediato: cero cambio de comportamiento del redactor de logs (ya bloqueaba el literal por nombre de
  columna, vía la hermana N2 de `0038`) — el valor real es que el serializador que la Tanda 4 escriba
  después hereda el tier correcto por default, en vez de heredar un N1 mal puesto. Verificado: typecheck
  limpio, `redactor.test.ts` (31/31) y `reglas-de-codigo.test.ts` (63/64, el único rojo es R-F,
  preexistente, confirmado con `git stash`).
